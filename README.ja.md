...
- Bot判定、組織分類、12種類の非同期管理画面
- 決定的な診断を備えた収集・分析除外ルール
- ストリーミングCSV・JSON、保持期間・安全な整理、保存状態診断
- 再開可能な再構築と長期混在レポートを備えた保持安全な日次集計
- City・ASN独立のGeoLite2診断、手動更新、安全な定期更新
- 英語・標準日本語のインターフェース
...
バージョン0.6.3では、IP完全一致、CIDR、URI、Native管理者、Bot、地域・ASN・組織、ブラウザ・OS・端末、参照元ドメイン、UTMルールの認証済み管理・診断機能を追加しました。収集スコープは今後の保存を防ぎ、分析スコープは一致する履歴を削除せず非表示にします。優先順位、照合、プライバシー、更新の詳細は[除外ルール](docs/EXCLUSIONS.ja.md)を参照してください。

## ログ寿命とエクスポート

バージョン0.7.0では、絞り込み済みのアクセスログ、セッション、コンテンツ、組織、流入元、キャンペーン、イベントをCSVまたは安定スキーマJSONで逐次出力します。IPは既定で省略し、マスク方式と明示確認付き生IP方式を選べます。保持期間は無期限・プリセット・検証済み任意日数に対応し、整理対象確認、排他ロック、制限付きトランザクション、再開状態、CLIスケジュール、保存状態診断を備えます。[ログ寿命、エクスポート、保持](docs/LOG_LIFECYCLE.ja.md)を参照してください。

## 日次集計

バージョン0.7.1では、完了済みローカル日の合計と、件数制限済みのコンテンツ、チャネル、参照元、キャンペーン、国、ASN・組織、イベント軸を保存します。ダッシュボードの日付レポートは、集計済み日と未集計の生ログ日を重ならない境界で合成します。再構築は冪等・チェックポイント付きで、1回31日まで処理して再開できます。削除対象の全日が集計済みになるまでcleanupは停止します。[日次集計と性能](docs/DAILY_AGGREGATION.ja.md)を参照してください。

## GeoLite2保守

バージョン0.8.1では、MaxMindの署名付きCloudflare R2配信に合わせて自動更新を強化します。Basic認証は固定MaxMindデータベースpermalinkだけへ送信し、自動リダイレクトを無効化します。各リダイレクトを文書化済みの完全一致HTTPS R2ホストで検証し、新しい認証情報なしのcURLハンドルで成果物を取得します。署名付きURLは秘密情報として扱い、ログ、ブラウザ応答、状態へ残しません。City・ASN独立の診断、安全な有効化、定期実行、手動アップロードは維持します。[GeoLite2自動更新](docs/GEOLITE2_UPDATES.ja.md)を参照してください。
...
診断は`php bin/doctor.php`、日次集計は`php bin/aggregate.php incremental`、期限到来時のGeoLite2更新は`php bin/geolite2-update.php scheduled`、保持整理は`php bin/cleanup.php`、その他は既存の認証情報・導入ツールを実行します。

## 以前のバージョンからの更新

バックアップ後、`config.php`、`data/`、`storage/`、`storage/installed.lock`を保持してアプリケーションファイルを上書きします。v0.8.0からv0.8.1への更新ではDBスキーマを変更しません。既存設定、導入ロック、管理者・DB認証情報、暗号鍵、サイトトークン、暗号化済みMaxMind認証情報、City・ASN MMDB、言語設定、解析、集計、除外、メタデータ、保存ビュー、ライフサイクル状態を保持します。

直帰率は1ページだけ閲覧した入口セッション数÷入口セッション数です。エンゲージメント平均は日別平均の平均ではなく、保持した合計と標本数から計算します。日別ユニーク訪問者は推定値で、別の日に同じ匿名ブラウザを再計上する場合があります。通知、複数サイト、権限は今後の課題です。
