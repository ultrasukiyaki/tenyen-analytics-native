...
バックアップ後、`config.php`、`data/`、`storage/`を保持してアプリケーションファイルを上書きします。0.6.0ではDB移行は不要です。localeキーのない設定を含め、既存のv0.5.7以前の設定とデータをそのまま利用できます。

0.6.0では、認証・CSRF保護されたセッションおよび匿名ブラウザの訪問経路画面を追加しました。直帰は「入口となったセッションのうち、ページビューがちょうど1件のもの」です。直帰率は直帰数÷入口数、離脱率はそのページが出口となったセッション数÷ページビュー数で、分母が0の場合は0%とします。
