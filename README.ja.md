...
バージョン0.8.0では、City・ASN独立の状態確認、暗号化したMaxMindライセンスキー保存、控えめな週次実行、今すぐ更新、排他、再試行バックオフ、検証済み展開、MMDBの原子的置換を追加します。失敗時は現在の有効DBを保持し、手動アップロードも引き続き利用できます。[GeoLite2自動更新](docs/GEOLITE2_UPDATES.ja.md)を参照してください。
...
バックアップ後、`config.php`、`data/`、`storage/`を保持してアプリケーションファイルを上書きします。v0.7.1からv0.8.0への更新ではDBスキーマを変更しません。既存設定、導入ロック、認証情報、鍵、言語設定、MMDB、解析・集計、除外、メタデータ、ライフサイクル状態を保持します。GeoLite2認証情報・更新状態は設定後にだけ保護済み`storage/`へ作成します。
