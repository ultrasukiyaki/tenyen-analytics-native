...
function upload_json(array $payload, int $status = 200): never
{
...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    upload_json(['ok' => false, 'message' => 'POST only.'], 405);
}

$root = dirname(__DIR__, 2);
if (is_file($root . '/config.php') || is_file($root . '/storage/installed.lock')) {
    upload_json(['ok' => false, 'message' => 'インストール済みのためアップロードを停止しました。'], 409);
}

$csrf = (string)($_POST['csrf'] ?? '');
$expectedCsrf = (string)($_SESSION['csrf'] ?? '');
if ($csrf === '' || $expectedCsrf === '' || !hash_equals($expectedCsrf, $csrf)) {
    upload_json(['ok' => false, 'message' => 'The installer session has expired. Reload the page.'], 419);
}

$kind = (string)($_POST['kind'] ?? '');
$filename = $kind === 'city' ? 'GeoLite2-City.mmdb' : ($kind === 'asn' ? 'GeoLite2-ASN.mmdb' : '');
if ($filename === '') {
    upload_json(['ok' => false, 'message' => 'MMDBの種類が不正です。'], 400);
}

$uploadId = strtolower((string)($_POST['upload_id'] ?? ''));
if (!preg_match('/^[a-f0-9-]{16,64}$/', $uploadId)) {
    upload_json(['ok' => false, 'message' => 'アップロードIDが不正です。'], 400);
}
...
if ($index === false || $totalChunks === false || $totalSize === false || $offset === false
    ...
    upload_json(['ok' => false, 'message' => 'アップロード情報が不正です。'], 400);
}
...
if (!is_array($chunk) || (int)($chunk['error'] ?? UPLOAD_ERR_NO_FILE) !== UPLOAD_ERR_OK) {
    ...
    upload_json(['ok' => false, 'message' => '分割ファイルを受信できませんでした（コード' . $code . '）。'], 400);
}
$chunkSize = (int)($chunk['size'] ?? 0);
if ($chunkSize < 1 || $chunkSize > 1024 * 1024) {
    upload_json(['ok' => false, 'message' => '分割サイズが不正です。'], 400);
}

$uploadDir = $root . '/storage/install-upload';
if (!is_dir($uploadDir) && !mkdir($uploadDir, 0700, true) && !is_dir($uploadDir)) {
    upload_json(['ok' => false, 'message' => 'storage/install-uploadを作成できません。'], 500);
}
if (!is_writable($uploadDir)) {
    upload_json(['ok' => false, 'message' => 'storage/install-uploadへ書き込めません。'], 500);
}

// Clean abandoned uploads older than one day.
...
if ($index > 0) {
    ...
    if (!is_array($decoded)) {
        upload_json(['ok' => false, 'message' => 'The upload state could not be restored. Start the upload again.'], 409);
    }
    $state = $decoded;
}
...
if ((int)$state['next_index'] !== $index || (int)$state['bytes'] !== $offset
    ...
    upload_json(['ok' => false, 'message' => 'The chunk sequence does not match. Start the upload again.'], 409);
}
...
if ($source === false || $target === false) {
    ...
    upload_json(['ok' => false, 'message' => '一時ファイルを作成できません。'], 500);
}
if (!flock($target, LOCK_EX)) {
    fclose($source); fclose($target);
    upload_json(['ok' => false, 'message' => 'アップロードファイルをロックできません。'], 500);
}
...
if ($written === false || $written !== $chunkSize) {
    upload_json(['ok' => false, 'message' => '分割ファイルの保存に失敗しました。'], 500);
}
...
$complete = $index + 1 === $totalChunks;
if (!$complete) {
    upload_json(['ok' => true, 'complete' => false, 'received' => $state['bytes'], 'total' => $totalSize]);
}

if ((int)$state['bytes'] !== $totalSize || filesize($partPath) !== $totalSize) {
    @unlink($partPath); @unlink($statePath);
    upload_json(['ok' => false, 'message' => 'The received size does not match. Upload the file again.'], 400);
}
...
if (!$inspection['ok']) {
    @unlink($partPath); @unlink($statePath);
    upload_json(['ok' => false, 'message' => $filename . '：' . $inspection['message']], 400);
}

$dataDir = $root . '/data';
if (!is_dir($dataDir) || !is_writable($dataDir)) {
    @unlink($partPath); @unlink($statePath);
    upload_json(['ok' => false, 'message' => 'The data directory is not writable. Check 755 or 775 permissions with FTP.'], 500);
}
$destination = $dataDir . '/' . $filename;
if (!@rename($partPath, $destination)) {
    if (!@copy($partPath, $destination)) {
        @unlink($partPath); @unlink($statePath);
        upload_json(['ok' => false, 'message' => $filename . 'をdataディレクトリへ保存できません。'], 500);
    }
    @unlink($partPath);
}
...
$_SESSION['draft']['geoip'][$kind] = true;
$_SESSION['geo_notice'] = $filename . '（' . (string)$inspection['type'] . '）を確認しました。';
