...
<?php elseif ($step === 2): ?>
<h2>解析するサイトを設定</h2><p class="lead">現在のURLから自動入力したで。共有サーバーで一式を置いた場合、公開URLの末尾には通常<code>/public</code>が入るで。</p>
...
<div class="field full"><label for="public_url">Tenyen Analytics公開URL</label><input id="public_url" name="public_url" type="url" required value="<?= ie($draft['public_url']) ?>"><div class="hint">config.js.php・tracker.js・collect.phpがあるpublicディレクトリのURLや。</div></div>
...
<div class="field"><label for="admin_password">管理パスワード</label><input id="admin_password" name="admin_password" type="password" minlength="12" required autocomplete="new-password"><div class="hint">12文字以上を推奨ではなく必須にしてるで。</div></div>
...
<div class="alert success"><strong>DBテーブル・config.php・秘密鍵・管理画面アカウントを作成したで。</strong></div>
...
<?php endif; ?>
</main></div><?php endif; ?><footer style="padding:20px;text-align:center;color:#64748b;font-size:12px">Tenyen Analytics v0.5.5 — <a href="https://www.10yendama.com/" target="_blank" rel="noopener noreferrer">Powered by 10yendama.com</a> — © <?= date('Y') > 2026 ? '2026–'.date('Y') : '2026' ?> 10yendama.com</footer></div>
<script>
