...
function tyaav_render(string $view, array $services, array $query): array
{
    ...
    if($view==='dashboard'){
        ...
        $bot=$pdo->prepare("SELECT COUNT(*) FROM tya_events WHERE occurred_at>=? AND occurred_at<? AND is_bot=1 AND ".ExclusionRules::analysisSql('tya_events'));$bot->execute([$range['start_utc'],$range['end_utc']]);$summary['bot_events']=(int)$bot->fetchColumn();
        ...
        $lifecycle=new LogLifecycle($pdo,$root.'/storage/lifecycle.json',(int)($config['app']['retention_days']??90));$diagnostics=$lifecycle->diagnostics();$lifeState=$diagnostics['state'];$retention=$lifeState['retention_days'];$cleanup=$lifeState['cleanup'];$csrf=tyaa_csrf_token();
