...
function tyaav_render(string $view, array $services, array $query): array
{
    ...
    $titles=['dashboard'=>tyaav_t('nav.dashboard'),'realtime'=>tyaav_t('nav.realtime'),'history'=>tyaav_t('nav.history'),'sessions'=>tyaav_t('nav.sessions'),'events'=>tyaav_t('nav.events'),'campaigns'=>tyaav_t('nav.campaigns'),'content'=>tyaav_t('nav.content'),'referrers'=>tyaav_t('nav.referrers'),'organizations'=>tyaav_t('nav.organizations'),'audience'=>tyaav_t('nav.audience'),'engagement'=>tyaav_t('nav.engagement'),'system'=>tyaav_t('nav.system'),'settings'=>tyaav_t('nav.settings')];
    ...
    if($view==='dashboard'){
        ...
        echo '<section class="view-head"><div><h1>'.tyaav_h(tyaav_t('nav.sessions')).'</h1><p>'.tyaav_h(tyaav_t('sessions.description')).'</p></div></section>'
            ...
            .'<label>'.tyaav_h(tyaav_t('filter.actor')).'<select name="actor"><option value="human">'.tyaav_h(tyaav_t('filter.human')).'</option><option value="bot">Bot</option><option value="all">'.tyaav_h(tyaav_t('filter.all')).'</option></select></label>'
            .'<label>'.tyaav_h(tyaav_t('sessions.country')).'<input name="country"></label><label>ASN / '.tyaav_h(tyaav_t('sessions.organization')).'<input name="organization"></label>'
            .'<label>'.tyaav_h(tyaav_t('sessions.browser')).'<input name="browser"></label><label>OS<input name="os"></label><label>'.tyaav_h(tyaav_t('sessions.device')).'<input name="device"></label>'
            .'<label>'.tyaav_h(tyaav_t('sessions.landing')).'<input name="content"></label>'
            .'<label>'.tyaav_h(tyaav_t('filter.per_page')).'<select name="per_page"><option>25</option><option>50</option><option>100</option></select></label>'
            .'<label>'.tyaav_h(tyaav_t('filter.order')).'<select name="order"><option value="desc">'.tyaav_h(tyaav_t('filter.newest')).'</option><option value="asc">'.tyaav_h(tyaav_t('filter.oldest')).'</option></select></label>'
        ...
        echo '<section class="view-head"><div><h1>'.tyaav_h(tyaav_t('nav.campaigns')).'</h1><p>First-touch campaign dimensions are taken from each session landing page.</p></div></section><section class="panel">'.tyaav_filter_html($range).'<div class="table-wrap"><table><thead><tr><th>Source</th><th>Medium</th><th>Campaign</th><th>Content</th><th>Term</th><th>Sessions</th><th>Visitors</th><th>PV</th><th>Events</th></tr></thead><tbody>';
        foreach($rows as $row)echo '<tr><td>'.tyaav_h($row['utm_source']).'</td><td>'.tyaav_h($row['utm_medium']).'</td><td class="journey-long">'.tyaav_h($row['utm_campaign']).'</td><td class="journey-long">'.tyaav_h($row['utm_content']).'</td><td class="journey-long">'.tyaav_h($row['utm_term']).'</td><td>'.(int)$row['sessions'].'</td><td>'.(int)$row['visitors'].'</td><td>'.(int)$row['pageviews'].'</td><td>'.(int)$row['events'].'</td></tr>';
        if(!$rows)echo '<tr><td colspan="9">'.tyaav_h(tyaav_t('common.no_data')).'</td></tr>';echo '</tbody></table></div></section>';
    } elseif($view==='content'){
        $rows=(new SessionAnalytics($pdo))->getContentJourneyMetrics(['actor'=>$range['actor'],'start_utc'=>$range['start_utc'],'end_utc'=>$range['end_utc']]);
        echo '<section class="view-head"><div><h1>'.tyaav_h(tyaav_t('nav.content')).'</h1><p>'.tyaav_h(tyaav_t('content.description')).'</p></div></section><section class="panel"><h2>'.tyaav_h(tyaav_t('content.ranking')).'</h2>'.tyaav_filter_html($range).'<p class="note">'.tyaav_h(tyaav_t('content.metric_note')).'</p><div class="table-wrap"><table><thead><tr><th>#</th><th>'.tyaav_h(tyaav_t('content.page')).'</th><th>PV</th><th>'.tyaav_h(tyaav_t('content.sessions')).'</th><th>'.tyaav_h(tyaav_t('content.entries')).'</th><th>'.tyaav_h(tyaav_t('content.exits')).'</th><th>'.tyaav_h(tyaav_t('content.bounces')).'</th><th>'.tyaav_h(tyaav_t('content.bounce_rate')).'</th><th>'.tyaav_h(tyaav_t('content.exit_rate')).'</th><th>'.tyaav_h(tyaav_t('content.pv_per_session')).'</th></tr></thead><tbody>';
        foreach($rows as $i=>$row){$entries=(int)$row['entries'];$pageviews=(int)$row['pageviews'];$sessions=(int)$row['sessions'];echo '<tr><td>'.($i+1).'</td><td><b>'.tyaav_page_link((string)$row['path'],(string)$row['page_title'],$siteUrl).'</b><br><code>'.tyaav_h($row['path']).'</code></td><td>'.number_format($pageviews).'</td><td>'.number_format($sessions).'</td><td>'.number_format($entries).'</td><td>'.number_format((int)$row['exits']).'</td><td>'.number_format((int)$row['bounces']).'</td><td>'.number_format($entries>0?(int)$row['bounces']/$entries*100:0,1).'%</td><td>'.number_format($pageviews>0?(int)$row['exits']/$pageviews*100:0,1).'%</td><td>'.number_format($sessions>0?$pageviews/$sessions:0,2).'</td></tr>';}if(!$rows)echo '<tr><td colspan="10">'.tyaav_h(tyaav_t('common.no_data')).'</td></tr>';echo '</tbody></table></div></section>';
        ...
        foreach($rows as $i=>$row){$label=(string)$row['referrer_host'];$render=$label==='Direct'?'Direct':tyaav_link((string)$row['sample_url'],$label);echo '<tr><td>'.($i+1).'</td><td>'.tyaav_h($row['traffic_channel']).'</td><td>'.$render.'</td><td>'.number_format((int)$row['pageviews']).'</td><td>'.number_format((int)$row['sessions']).'</td><td>'.number_format((int)$row['visitors']).'</td></tr>';}if(!$rows)echo '<tr><td colspan="6">No data</td></tr>';echo '</tbody></table></div></section><section class="panel"><h2>External clicks and downloads</h2><div class="table-wrap"><table><thead><tr><th>Type</th><th>Target URL</th><th>Events</th></tr></thead><tbody>';
