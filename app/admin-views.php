...
{
    $alias = $alias !== '' ? rtrim($alias, '.') : 'tya_events';
    $prefix = $alias . '.';
    ...
    $sql .= ' AND ' . ExclusionRules::analysisSql($alias);
...
function tyaav_history_options(PDO $pdo): array
{
    ...
    foreach(['countries'=>['country_name',100],'browsers'=>['browser',60],'os'=>['os',60],'devices'=>['device_type',60]] as $key=>[$column,$limit]){
        $rows=$pdo->query("SELECT {$column} AS value,COUNT(*) AS hits FROM tya_events WHERE {$column}<>'' AND ".ExclusionRules::analysisSql('tya_events')." GROUP BY {$column} ORDER BY hits DESC,{$column} ASC LIMIT ".(int)$limit)->fetchAll();
        foreach($rows as $row)$options[$key][]=['value'=>(string)$row['value'],'label'=>(string)$row['value']];
    }
    return $options;
}

function tyaav_mmdb_upload_panel(array $services): string
{
    $csrf = tyaa_csrf_token();
    $updater=new GeoLite2Updater($services['root'],$services['config'],$services['crypto']);$status=$updater->publicStatus();$settings=$updater->settings();$cards='';
    foreach(['city'=>'City','asn'=>'ASN'] as $kind=>$label){$db=$status['state'][$kind];$cards.='<div class="card">GeoLite2 '.$label.'<b>'.tyaav_h($db['health']??'missing').'</b><small>'.tyaav_h(($db['filename']??'—').' / '.($db['build_date']??'—').' / '.number_format((int)($db['size']??0)).' bytes').'</small><small>'.tyaav_h(($db['last_success']??'—').' / '.($db['error']??'')).'</small></div>';}
    return '<section class="panel"><h2>'.tyaav_h(tyaav_t('geoip.automatic')).'</h2><div class="cards">'.$cards.'</div><form class="filters" data-geolite2-form><label>'.tyaav_h(tyaav_t('geoip.account_id')).'<input name="account_id" inputmode="numeric" maxlength="20" required value="'.tyaav_h($settings['account_id']).'"></label><label>'.tyaav_h(tyaav_t('geoip.license_key')).'<input name="license_key" type="password" maxlength="80" placeholder="'.tyaav_h($settings['license_mask']).'"></label><label><input name="enabled" type="checkbox" value="1"'.(!empty($settings['enabled'])?' checked':'').'> '.tyaav_h(tyaav_t('geoip.enable')).'</label><button class="button" type="submit">'.tyaav_h(tyaav_t('common.save')).'</button><small data-geolite2-status></small></form><div class="dialog-actions"><button class="button" type="button" data-geolite2-action="update_all">'.tyaav_h(tyaav_t('geoip.update_now')).'</button><button class="button secondary" type="button" data-geolite2-action="update_kind" data-kind="city">City</button><button class="button secondary" type="button" data-geolite2-action="update_kind" data-kind="asn">ASN</button></div><p class="note">'.tyaav_h(tyaav_t('geoip.schedule_note',['next'=>(string)($status['state']['next_run']??'—')])).'</p><pre class="code-block" data-geolite2-result hidden></pre></section><section class="panel"><h2>'.tyaav_h(tyaav_t('geoip.manual')).'</h2>'
...
function tyaav_render(string $view, array $services, array $query): array
{
    ...
    $root=$services['root'];$pdo=$services['pdo'];$config=$services['config'];$geoIp=$services['geoIp'];
    ...
    $range=tyaav_range($query,$timezone);$scope=tyaav_scope($range);$eScope=tyaav_scope($range,'e');$pScope=tyaav_scope($range,'p');
    $chart=[];$history=null;$refresh=0;
    $titles=['dashboard'=>tyaav_t('nav.dashboard'),'realtime'=>tyaav_t('nav.realtime'),'history'=>tyaav_t('nav.history'),'sessions'=>tyaav_t('nav.sessions'),'events'=>tyaav_t('nav.events'),'campaigns'=>tyaav_t('nav.campaigns'),'content'=>tyaav_t('nav.content'),'referrers'=>tyaav_t('nav.referrers'),'organizations'=>tyaav_t('nav.organizations'),'metadata'=>tyaav_t('nav.metadata'),'exclusions'=>tyaav_t('nav.exclusions'),'lifecycle'=>tyaav_t('nav.lifecycle'),'audience'=>tyaav_t('nav.audience'),'engagement'=>tyaav_t('nav.engagement'),'system'=>tyaav_t('nav.system'),'settings'=>tyaav_t('nav.settings')];
    ...
    if($view==='dashboard'){
        ...
        $aggregateMode='raw';
        if($grain!=='hour'){$aggregation=new DailyAggregation($pdo,$timezone);$aggregateReport=$aggregation->report($range['from'],$range['to'],$range['actor']);$summary=$aggregateReport['summary'];$botReport=$aggregation->report($range['from'],$range['to'],'bot');$summary['bot_events']=(int)$botReport['summary']['source_event_count'];$dailyRows=$aggregateReport['timeline'];if($grain==='month'){$months=[];foreach($dailyRows as $row){$key=substr((string)$row['bucket'],0,7).'-01';foreach(['pageviews','visitors','sessions'] as $metric)$months[$key][$metric]=(int)($months[$key][$metric]??0)+(int)($row[$metric]??0);$months[$key]['bucket']=$key;}$dailyRows=array_values($months);}$timelineRows=tyaav_fill_timeline($dailyRows,$range['start_local'],$range['end_local'],$grain);$aggregateMode=$aggregateReport['mode'];}
        else{$stmt=$pdo->prepare("SELECT SUM(event_type='pageview') pageviews,COUNT(DISTINCT CASE WHEN event_type='pageview' THEN NULLIF(visitor_id,'') END) visitors,COUNT(DISTINCT CASE WHEN event_type='pageview' THEN NULLIF(session_id,'') END) sessions,AVG(CASE WHEN event_type='engagement' AND duration_ms>0 THEN duration_ms END) avg_duration_ms,AVG(CASE WHEN event_type='engagement' THEN scroll_depth END) avg_scroll FROM tya_events WHERE {$scope['sql']}");$stmt->execute($scope['params']);$summary=$stmt->fetch()?:[];$bot=$pdo->prepare("SELECT COUNT(*) FROM tya_events WHERE occurred_at>=? AND occurred_at<? AND is_bot=1 AND ".ExclusionRules::analysisSql('tya_events'));$bot->execute([$range['start_utc'],$range['end_utc']]);$summary['bot_events']=(int)$bot->fetchColumn();$timeline=$pdo->prepare("SELECT {$bucket} bucket,COUNT(*) pageviews,COUNT(DISTINCT NULLIF(visitor_id,'')) visitors,COUNT(DISTINCT NULLIF(session_id,'')) sessions FROM tya_events WHERE event_type='pageview' AND {$scope['sql']} GROUP BY {$bucket} ORDER BY {$bucket}");$timeline->execute($scope['params']);$timelineRows=tyaav_fill_timeline($timeline->fetchAll(),$range['start_local'],$range['end_local'],$grain);}
        ...
        $recent=$pdo->query("SELECT * FROM tya_events WHERE event_type='pageview' AND is_bot=0 AND ".ExclusionRules::analysisSql('tya_events')." ORDER BY event_id DESC LIMIT 8")->fetchAll();
        $candidates=$pdo->query("SELECT * FROM tya_events WHERE event_type='pageview' AND is_bot=0 AND asn_org<>'' AND ".ExclusionRules::analysisSql('tya_events')." ORDER BY event_id DESC LIMIT 120")->fetchAll();$notable=[];
        ...
        echo '<section class="panel"><h2>期間分析</h2>'.tyaav_filter_html($range).'<p class="note">'.tyaav_h(tyaav_t('aggregation.report_source',['source'=>$aggregateMode])).'</p><div class="cards">';
        ...
        $search=trim((string)($query['q']??''));$watched=(string)($query['watched']??'');$tagId=max(0,(int)($query['tag_id']??0));$where="e.event_type='pageview' AND {$eScope['sql']}";$params=$eScope['params'];
        ...
        $stmt=$pdo->prepare("SELECT p.path,MAX(p.page_title) page_title,COUNT(*) pageviews,COALESCE(AVG((SELECT MAX(e.duration_ms) FROM tya_events e WHERE e.session_id=p.session_id AND e.path=p.path AND e.event_type='engagement' AND e.occurred_at>=p.occurred_at AND ".ExclusionRules::analysisSql('e')."),0) avg_duration,COALESCE(AVG((SELECT MAX(e2.scroll_depth) FROM tya_events e2 WHERE e2.session_id=p.session_id AND e2.path=p.path AND e2.event_type='engagement' AND e2.occurred_at>=p.occurred_at AND ".ExclusionRules::analysisSql('e2')."),0) avg_scroll FROM tya_events p WHERE p.event_type='pageview' AND {$pScope['sql']} GROUP BY p.path ORDER BY pageviews DESC LIMIT 100");$stmt->execute($pScope['params']);$rows=$stmt->fetchAll();
    ...
    } elseif($view==='lifecycle'){
        $lifecycle=new LogLifecycle($pdo,$root.'/storage/lifecycle.json',(int)($config['app']['retention_days']??90),$timezone->getName());$diagnostics=$lifecycle->diagnostics();$lifeState=$diagnostics['state'];$retention=$lifeState['retention_days'];$cleanup=$lifeState['cleanup'];$aggregation=$diagnostics['aggregation'];$csrf=tyaa_csrf_token();
        echo '<section class="view-head"><div><h1>'.tyaav_h(tyaav_t('nav.lifecycle')).'</h1><p>'.tyaav_h(tyaav_t('lifecycle.description')).'</p></div></section>';
        echo '<section class="panel"><h2>'.tyaav_h(tyaav_t('lifecycle.export')).'</h2><form class="filters" method="post" action="api/export.php" data-export-form><input type="hidden" name="csrf" value="'.tyaav_h($csrf).'"><label>'.tyaav_h(tyaav_t('lifecycle.dataset')).'<select name="dataset">';foreach(['access','sessions','content','organizations','traffic_sources','campaigns','events'] as $dataset)echo '<option value="'.$dataset.'">'.$dataset.'</option>';echo '</select></label><label>'.tyaav_h(tyaav_t('lifecycle.format')).'<select name="format"><option value="csv">CSV</option><option value="json">JSON</option></select></label><label>'.tyaav_h(tyaav_t('lifecycle.ip_mode')).'<select name="ip_mode"><option value="omit">omit</option><option value="masked">masked</option><option value="raw">raw</option></select></label><label>'.tyaav_h(tyaav_t('filter.from')).'<input type="date" name="from"></label><label>'.tyaav_h(tyaav_t('filter.to')).'<input type="date" name="to"></label><label>'.tyaav_h(tyaav_t('filter.actor')).'<select name="actor"><option value="all">'.tyaav_h(tyaav_t('filter.all')).'</option><option value="human">'.tyaav_h(tyaav_t('filter.human')).'</option><option value="bot">Bot</option></select></label><label>Traffic source<input name="source" maxlength="32"></label><label>Campaign<input name="campaign" maxlength="255"></label><label>Event<input name="event" maxlength="32"></label><label>Content path<input name="content" maxlength="255"></label><label>Country<input name="country" maxlength="2"></label><label>ASN<input name="asn" type="number" min="1"></label><label>Organization<input name="organization" maxlength="255"></label><label>'.tyaav_h(tyaav_t('metadata.watchlist')).'<select name="watched"><option value="">'.tyaav_h(tyaav_t('filter.all')).'</option><option value="1">'.tyaav_h(tyaav_t('metadata.watched')).'</option><option value="0">'.tyaav_h(tyaav_t('metadata.unwatched')).'</option></select></label><label>Tag ID<input name="tag_id" type="number" min="1"></label><label>'.tyaav_h(tyaav_t('lifecycle.raw_confirmation')).'<input name="raw_confirmation" maxlength="13" placeholder="EXPORT_RAW_IP"></label><button class="button" type="submit">'.tyaav_h(tyaav_t('lifecycle.download')).'</button></form><p class="note">'.tyaav_h(tyaav_t('lifecycle.export_note')).'</p></section>';
        echo '<section class="panel"><h2>'.tyaav_h(tyaav_t('lifecycle.retention')).'</h2><form class="filters" data-lifecycle-form data-action="save_retention"><label>'.tyaav_h(tyaav_t('lifecycle.retention')).'<select name="retention"><option value="unlimited"'.($retention===null?' selected':'').'>unlimited</option>';foreach([30,90,180,365] as $days)echo '<option value="'.$days.'"'.($retention===$days?' selected':'').'>'.$days.' days</option>';echo '<option value="custom"'.($retention!==null&&!in_array($retention,[30,90,180,365],true)?' selected':'').'>custom</option></select></label><label>'.tyaav_h(tyaav_t('lifecycle.custom_days')).'<input type="number" name="custom_days" min="1" max="3650" value="'.($retention!==null&&!in_array($retention,[30,90,180,365],true)?(int)$retention:'').'"></label><button class="button" type="submit">'.tyaav_h(tyaav_t('common.save')).'</button><small data-lifecycle-status></small></form><p class="notice">'.tyaav_h(tyaav_t('lifecycle.raw_warning')).'</p><div class="dialog-actions"><button class="button secondary" type="button" data-lifecycle-action="preview">'.tyaav_h(tyaav_t('lifecycle.preview')).'</button><button class="button" type="button" data-lifecycle-action="cleanup">'.tyaav_h(tyaav_t('lifecycle.cleanup_now')).'</button></div><pre class="code-block" data-lifecycle-result hidden></pre></section>';
        echo '<section class="panel"><h2>'.tyaav_h(tyaav_t('aggregation.title')).'</h2><div class="cards"><div class="card">'.tyaav_h(tyaav_t('aggregation.status')).'<b>'.tyaav_h($aggregation['status']??'never').'</b></div><div class="card">'.tyaav_h(tyaav_t('aggregation.complete_days')).'<b>'.number_format((int)($aggregation['complete_days']??0)).'</b></div><div class="card">'.tyaav_h(tyaav_t('aggregation.coverage')).'<b>'.tyaav_h(($aggregation['first_complete_day']??'—').' – '.($aggregation['last_complete_day']??'—')).'</b></div></div><form class="filters" data-lifecycle-form data-action="aggregate_range"><label>'.tyaav_h(tyaav_t('filter.from')).'<input type="date" name="from" required></label><label>'.tyaav_h(tyaav_t('filter.to')).'<input type="date" name="to" required></label><button class="button secondary" type="submit">'.tyaav_h(tyaav_t('aggregation.rebuild')).'</button></form><div class="dialog-actions"><button class="button" type="button" data-lifecycle-action="aggregate_incremental">'.tyaav_h(tyaav_t('aggregation.incremental')).'</button><button class="button secondary" type="button" data-lifecycle-action="aggregate_resume">'.tyaav_h(tyaav_t('aggregation.resume')).'</button></div><p class="note">'.tyaav_h(tyaav_t('aggregation.note')).'</p><pre class="code-block" data-lifecycle-result hidden></pre></section>';
        echo '<section class="panel"><h2>'.tyaav_h(tyaav_t('lifecycle.storage')).'</h2><div class="cards"><div class="card">'.tyaav_h(tyaav_t('lifecycle.database_size')).'<b>'.tyaav_h(number_format((int)$diagnostics['database_bytes']).' bytes').'</b></div><div class="card">'.tyaav_h(tyaav_t('lifecycle.event_table_size')).'<b>'.tyaav_h(number_format((int)$diagnostics['event_table_bytes']).' bytes').'</b></div><div class="card">'.tyaav_h(tyaav_t('lifecycle.events')).'<b>'.number_format((int)$diagnostics['events']).'</b></div><div class="card">'.tyaav_h(tyaav_t('lifecycle.sessions')).'<b>'.number_format((int)$diagnostics['sessions']).'</b></div><div class="card">'.tyaav_h(tyaav_t('lifecycle.oldest')).'<b>'.tyaav_h($diagnostics['oldest']??'—').'</b></div><div class="card">'.tyaav_h(tyaav_t('lifecycle.newest')).'<b>'.tyaav_h($diagnostics['newest']??'—').'</b></div><div class="card">'.tyaav_h(tyaav_t('lifecycle.cleanup_status')).'<b>'.tyaav_h($cleanup['status']??'never').'</b><small>'.tyaav_h(($cleanup['last_run']??'—').' / '.($cleanup['next_run']??'—')).'</small></div></div><div class="table-wrap"><table><thead><tr><th>'.tyaav_h(tyaav_t('lifecycle.month')).'</th><th>'.tyaav_h(tyaav_t('lifecycle.events')).'</th></tr></thead><tbody>';foreach($diagnostics['monthly'] as $month)echo '<tr><td>'.tyaav_h($month['month']).'</td><td>'.number_format((int)$month['events']).'</td></tr>';if(!$diagnostics['monthly'])echo '<tr><td colspan="2">'.tyaav_h(tyaav_t('common.no_data')).'</td></tr>';echo '</tbody></table></div><p class="note"><code>php bin/cleanup.php scheduled</code> — '.tyaav_h(tyaav_t('lifecycle.cron_note')).'</p></section>';
    } elseif($view==='exclusions'){
        $rules=new ExclusionRules($pdo,$services['crypto'],$overrides);$items=$rules->list(1,100);$csrf=tyaa_csrf_token();
        echo '<section class="view-head"><div><h1>'.tyaav_h(tyaav_t('nav.exclusions')).'</h1><p>'.tyaav_h(tyaav_t('exclusions.description')).'</p></div></section>';
        echo '<section class="panel"><h2>'.tyaav_h(tyaav_t('exclusions.add')).'</h2><form class="filters" data-exclusion-form><input type="hidden" name="rule_id" value=""><label>'.tyaav_h(tyaav_t('exclusions.type')).'<select name="rule_type">';
        foreach(ExclusionRules::TYPES as $type)echo '<option value="'.tyaav_h($type).'">'.tyaav_h($type).'</option>';
        echo '</select></label><label>'.tyaav_h(tyaav_t('exclusions.value')).'<input name="rule_value" maxlength="255" required></label><label>'.tyaav_h(tyaav_t('exclusions.scope')).'<select name="scope"><option value="collection">collection</option><option value="analysis">analysis</option><option value="both">both</option></select></label><label><input type="checkbox" name="enabled" value="1" checked> '.tyaav_h(tyaav_t('exclusions.enabled')).'</label><label>'.tyaav_h(tyaav_t('exclusions.note')).'<input name="note" maxlength="1000"></label><button class="button" type="submit">'.tyaav_h(tyaav_t('common.save')).'</button><small data-exclusion-status></small></form><p class="note">'.tyaav_h(tyaav_t('exclusions.scope_note')).'</p></section>';
        echo '<section class="panel"><h2>'.tyaav_h(tyaav_t('exclusions.rules')).'</h2><div class="table-wrap"><table><thead><tr><th>ID</th><th>'.tyaav_h(tyaav_t('exclusions.type')).'</th><th>'.tyaav_h(tyaav_t('exclusions.value')).'</th><th>'.tyaav_h(tyaav_t('exclusions.scope')).'</th><th>'.tyaav_h(tyaav_t('exclusions.enabled')).'</th><th>'.tyaav_h(tyaav_t('exclusions.note')).'</th><th>'.tyaav_h(tyaav_t('exclusions.timestamps')).'</th><th>'.tyaav_h(tyaav_t('common.edit')).'</th></tr></thead><tbody>';
        foreach($items as $item){$encoded=htmlspecialchars(json_encode($item,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES|JSON_HEX_TAG|JSON_HEX_AMP|JSON_HEX_APOS|JSON_HEX_QUOT),ENT_QUOTES|ENT_SUBSTITUTE,'UTF-8');echo '<tr><td>'.(int)$item['rule_id'].'</td><td><code>'.tyaav_h($item['rule_type']).'</code></td><td>'.tyaav_h($item['rule_value']).'</td><td>'.tyaav_h($item['scope']).' / '.tyaav_h($item['action']).'<br><small>precedence '.(int)$item['precedence'].'</small></td><td>'.(!empty($item['enabled'])?tyaav_h(tyaav_t('settings.enabled')):tyaav_h(tyaav_t('settings.disabled'))).'</td><td>'.tyaav_h($item['note']).'</td><td>'.tyaav_h($item['created_at']).'<br>'.tyaav_h($item['updated_at']).'</td><td><button class="button secondary" type="button" data-edit-exclusion data-rule="'.$encoded.'">'.tyaav_h(tyaav_t('common.edit')).'</button> <button class="button secondary" type="button" data-delete-exclusion="'.(int)$item['rule_id'].'">'.tyaav_h(tyaav_t('common.delete')).'</button></td></tr>';}
        if(!$items)echo '<tr><td colspan="8">'.tyaav_h(tyaav_t('common.no_data')).'</td></tr>';echo '</tbody></table></div></section>';
        $sample=json_encode(['ip'=>'203.0.113.10','path'=>'/preview','is_bot'=>false,'country_code'=>'JP','asn'=>64500,'asn_org'=>'Example Corporation','browser'=>'Chrome','os'=>'Linux','device_type'=>'desktop','referrer_domain'=>'example.com','utm_source'=>'newsletter'],JSON_PRETTY_PRINT|JSON_UNESCAPED_SLASHES);
        echo '<section class="panel"><h2>'.tyaav_h(tyaav_t('exclusions.diagnostic')).'</h2><form data-exclusion-diagnostic><label>'.tyaav_h(tyaav_t('exclusions.scope')).'<select name="scope"><option value="collection">collection</option><option value="analysis">analysis</option></select></label><label>'.tyaav_h(tyaav_t('exclusions.context')).'<textarea name="context_json" rows="10" maxlength="8192">'.tyaav_h($sample).'</textarea></label><button class="button" type="submit">'.tyaav_h(tyaav_t('exclusions.test')).'</button><pre class="code-block" data-exclusion-result hidden></pre></form></section>';
    } elseif($view==='system'){
        ...
        echo tyaav_mmdb_upload_panel($services);
    }
