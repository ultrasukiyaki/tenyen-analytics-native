...
function tyaav_render(string $view, array $services, array $query): array
{
    ...
    if($view==='dashboard'){
        ...
        echo '<section class="view-head"><div><h1>ダッシュボード</h1><p>サイト全体の動きを短く確認する入口や。</p></div><a class="button secondary" href="?view=history" data-view-link="history">詳細履歴へ</a></section>';
        ...
        if(!$notable)echo '<p class="muted">注目組織のアクセスはまだないで。</p>';
        ...
        if(!$recent)echo '<tr><td colspan="4">まだデータがないで。</td></tr>';echo '</tbody></table></div></section>';
        ...
        if(!$rows)echo '<tr><td colspan="6">この時間帯のアクセスはまだないで。</td></tr>';echo '</tbody></table></div></section>';$refresh=30;
    } elseif($view==='history'){
        ...
        echo '<section class="view-head"><div><h1>コンテンツ</h1><p>記事・固定ページごとの閲覧状況や。</p></div></section><section class="panel"><h2>記事ランキング</h2>'.tyaav_filter_html($range).'<div class="table-wrap"><table><thead><tr><th>#</th><th>ページ</th><th>PV</th><th>UU</th><th>セッション</th></tr></thead><tbody>';
        ...
        echo '<section class="view-head"><div><h1>ASN・組織</h1><p>接続元IPに登録されたネットワーク組織を分類する、Tenyen Analyticsの中核画面や。</p></div></section><section class="panel"><h2>ASN・組織ランキング</h2>'.tyaav_filter_html($range).'<p class="note">組織名は閲覧者個人の所属先を断定するものではないで。</p><div class="table-wrap"><table><thead><tr><th>#</th><th>分類</th><th>ASN／登録組織</th><th>PV</th><th>UU</th><th>セッション</th><th>最終アクセス</th></tr></thead><tbody>';
